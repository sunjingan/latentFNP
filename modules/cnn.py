...
class SpectralConv2d_fast(nn.Module):
    def __init__(self, in_channels, out_channels, modes):
        ...
        self.weights1 = nn.Parameter(self.scale * torch.rand(2, in_channels, out_channels, self.modes, self.modes))
        self.weights2 = nn.Parameter(self.scale * torch.rand(2, in_channels, out_channels, self.modes, self.modes))
    ...
    def forward(self, x):
        ...
        out_ft_real[:, :, :self.modes, :self.modes] = \
            self.compl_mul2d(x_ft[:, :, :self.modes, :self.modes].real, self.weights1[0]) - self.compl_mul2d(x_ft[:, :, :self.modes, :self.modes].imag, self.weights1[1])
        out_ft_real[:, :, -self.modes:, :self.modes] = \
            self.compl_mul2d(x_ft[:, :, -self.modes:, :self.modes].real, self.weights2[0]) - self.compl_mul2d(x_ft[:, :, -self.modes:, :self.modes].imag, self.weights2[1])
        
        out_ft_imag[:, :, :self.modes, :self.modes] = \
            self.compl_mul2d(x_ft[:, :, :self.modes, :self.modes].imag, self.weights1[0]) + self.compl_mul2d(x_ft[:, :, :self.modes, :self.modes].real, self.weights1[1])
        out_ft_imag[:, :, -self.modes:, :self.modes] = \
            self.compl_mul2d(x_ft[:, :, -self.modes:, :self.modes].imag, self.weights2[0]) + self.compl_mul2d(x_ft[:, :, -self.modes:, :self.modes].real, self.weights2[1])        
...
class FCNN(nn.Module):
    ...
    def __init__(self, n_channels, ConvBlock, n_blocks=3, is_chan_last=False, **kwargs):
        ...
        self.fno_blocks = nn.ModuleList(
            [
                SpectralConv2d_fast(in_chan, out_chan, modes=12)
                for in_chan, out_chan in self.in_out_channels
            ]
        )
        self.is_return_rep = False  # never return representation for vanilla conv
